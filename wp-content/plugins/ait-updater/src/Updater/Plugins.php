...
class Plugins extends Base
{
	...
	public function getPluginInfoCallback($false, $action, $arg)
	{
		...
		if(isset($arg->slug) and (substr($arg->slug, 0, 4) === 'ait-' or isset($aitPlugins[$arg->slug]))){
			...
			$apiResponse = $this->updater->getApiClient()->getModule('plugins')->getPlugin($codename);
			...
			if(!$apiResponse->isSuccessful()){
				...
				if($apiResponse->getResponseCode() == 400){ // better UX
					...
					$error = new \WP_Error('download_failed', '<div class="ait-updater-purchase-message notice notice-warning">' .
						sprintf(
							__('Oops, you haven’t purchased this plugin yet. Get access to all plugins, themes and graphics by <a href="%s" target="_blank">upgrading to our PremiumX Membership</a>.', 'ait-updater'),
							'https://www.ait-themes.club/pricing-sign-up/'
						) . '</div>'
					);
				}else{
