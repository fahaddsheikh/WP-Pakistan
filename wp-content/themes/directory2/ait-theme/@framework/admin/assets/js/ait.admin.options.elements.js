...
(function($, $window, $document, undefined)
{
	...
	var ui = ait.admin.options.elements.Ui = {
		...
		updateContentPreview: function(previewContent, optId, currentLocale) {
			var layout = data.getOpt(optId, currentLocale, 'layout').val(),
				columns = parseInt(data.getOpt(optId, currentLocale, layout + 'columns').val()),
				carousel = parseInt(data.getOpt(optId, currentLocale, layout + 'enablecarousel').val()),
				rows = parseInt(data.getOpt(optId, currentLocale, layout + 'rows').val()),
				rows = carousel == 1 ? rows : 1;

			/* Fallback if not typical item organizer */
			var layout = (typeof layout === 'undefined') ? previewContent.find('.ait-element-placeholder-wrap').data('layout') : layout,
				columns = isNaN(columns) ? (layout == 'box' ? parseInt(previewContent.find('.ait-element-placeholder').length) : 1) : columns,
				rows = (isNaN(carousel) && rows == 1) ? parseInt(data.getOpt(optId, currentLocale, 'count').val()) : rows,
				rows = rows > 4 ? 4 : rows;
			/* Fallback if not typical item organizer */
			...
			$placeholders
				.removeClass()
				.addClass('ait-element-placeholder-wrap layout-' + layout)
