	...
	switch($bep_selectedtype) {
		...
		case 'ait-event-pro':
			...
			if($module_type == 'biggrid') {
				$biggrid_image_src_link =  get_post_meta(get_the_id(), '_ait-event-pro_event-pro-data', true);
				$image_src_link 		= "<img width='" . $image_width . "' height='" . $image_height . "' src='" . $biggrid_image_src_link['headerImage'] . "'>";	
			}
			elseif ($module_type == 'normal') {
				$image_src_link 		= "<span class='event-date'>" . $bep_eventfrom_date . "<sup>" . $bep_eventfrom_date_suffix . "</sup></span>";
				$image_src_link 	   .= "<span class='event-month'>" . $bep_eventfrom_month . "</span>";	
			}
			break;
		default: 
...
function bep_custom_author_name($module_type = '') {
